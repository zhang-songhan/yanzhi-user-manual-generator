...
description: Use when the user provides a web frontend project source and a user manual with screenshot placeholders (【图X：...】) and wants to automatically capture real screenshots by starting the web app and using Chrome DevTools MCP. Triggers on keywords like "auto capture screenshots", "fill screenshot placeholders", "capture screenshots for manual", "自动截图", "补充截图", "批量截图".
---
...
Automatically capture real screenshots for a user manual by: parsing screenshot placeholders from the manual, starting the web application, navigating to each relevant page with Chrome DevTools MCP, and saving screenshots to the manual's `screenshots/` directory with correct filenames.
...
**Verify Chrome DevTools MCP is available.** Check the available MCP tools list. The skill requires `mcp__plugin_chrome-devtools-mcp_chrome-devtools__navigate_page` and related tools. If Chrome DevTools MCP tools are not found:

> Chrome DevTools MCP 不可用。auto-capture-for-webapp 依赖 Chrome DevTools MCP 进行浏览器截图。请确保已安装并配置 Chrome DevTools MCP server（插件名: chrome-devtools-mcp）。

STOP if Chrome DevTools MCP is unavailable — do not attempt to use alternative screenshot methods.
...
> 该项目不包含纯 web 前端的 GUI。auto-capture-for-webapp 仅支持 web 前端项目。对于 CLI、桌面应用、移动端应用等非 web GUI，无法使用 Chrome DevTools MCP 进行截图。
...
digraph auto_capture {
  ...
  "Navigate to page with Chrome DevTools" [shape=box];
  ...
  "For each screenshot task" -> "Navigate to page with Chrome DevTools";
  "Navigate to page with Chrome DevTools" -> "Interact if needed (fill forms, click)";
...
4. **Wait for the server to be ready** — poll the expected port until it responds (use `curl` or a Chrome DevTools navigation attempt)
...
**CRITICAL: NEVER skip the readiness check.** Chrome DevTools navigation will fail silently if the server isn't running.

## Step 4: Capture Screenshots with Chrome DevTools MCP

For each screenshot task, in order:

### 4.1 Navigate to the Page

Use `mcp__plugin_chrome-devtools-mcp_chrome-devtools__navigate_page` to go to the target URL.

```json
{"type": "url", "url": "http://localhost:PORT/route"}
```
...
4. Wait for redirect to complete (`wait_for` with expected text)
...
Use Chrome DevTools MCP tools to achieve the required state:
- `fill_form` — fill multiple form fields at once (preferred over individual fills)
- `click` — click buttons, links
- `fill` — type into a single input or select dropdown options
- `type_text` — type text with keyboard into a focused input
- `wait_for` — wait for specific text/element to appear

### 4.4 Take the Screenshot

Use `take_screenshot` to capture the current page state. **Always use `fullPage: false`** for standard viewport screenshots. Use `fullPage: true` ONLY if the placeholder description explicitly mentions "全页" or "长页面".
...
Use the `filePath` parameter in `take_screenshot` to save directly to the target path, or **rename/move** the output to the correct path:
...
Some apps have authentication flows that Chrome DevTools cannot automate:

| Auth Type | Action |
|-----------|--------|
| Username/password | Automate with `fill_form` + `click` |
...
**If Chrome DevTools cannot complete login automatically:** Open the login page, ask the user to complete authentication manually, then continue with automated screenshot capture.
